...
{
	@SubscribeEvent
	public void onLivingTick(PlayerTickEvent.Pre e)
	{
		Player living = e.getEntity();
		...
		if (living.canDrownInFluidType(living.getEyeInFluidType()) && !MobEffectUtil.hasWaterBreathing(living))
		{
			...
			if (maxAirSupply - airSupply >= airDuration)
			{
				...
				int airUsing = AddonPneumaticCraftConfig.UPGRADES_OXYGEN_PROOF.airUsing;
	...
	public int onHotTemperatureProof(Entity entity)
	{
		if (entity instanceof Player living)
		{
			...
			int airUsing = AddonPneumaticCraftConfig.UPGRADES_HOT_TEMPERATURE_PROOF.airUsing;
	...
	public int onAcidRainProof(Entity entity)
	{
		if (entity instanceof Player living)
		{
			...
			int airUsing = AddonPneumaticCraftConfig.UPGRADES_ACID_RAIN_PROOF.airUsing;
			return this.useAir(living, upgradeHandler, airUsing, false) ? ProofAbstractUtils.GENERAL_PROOF_INTERVAL : 0;
		}
		else
		{
	...
	public int onGravityProof(Entity entity)
	{
		if (entity instanceof Player living)
		{
			...
			int airUsing = AddonPneumaticCraftConfig.UPGRADES_GRAVITY_PROOF.airUsing;
			return this.useAir(living, upgradeHandler, airUsing, false) ? ProofAbstractUtils.GENERAL_PROOF_INTERVAL : 0;
		}
		else
		{
	...
	public boolean useOxygen(Player living, IArmorUpgradeHandler<?> upgradeHandler, long oxygenUsing, boolean simulate)
	{
	...
	public IAirHandlerItem getUsableAirHandler(LivingEntity living, IArmorUpgradeHandler<?> upgradeHandler)
	{
		if (living instanceof Player player)
		{
			...
			if (commonHandler.upgradeUsable(upgradeHandler, true) && stack.getItem() instanceof PneumaticArmorItem)
			{
				return stack.getCapability(PNCCapabilities.AIR_HANDLER_ITEM);
			}
