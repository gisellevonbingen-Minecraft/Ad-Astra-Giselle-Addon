...
public class OxygenChargerUtils
{
	...
	{
		List<ItemStackReference> items = LivingEntityHelper.getInventoryStacks(living);
		long stored = 0L;
		long capacity = 0L;
	...
	{
		return LivingEntityHelper.getInventoryStacks(living).stream().map(OxygenChargerUtils::get).filter(oxygenCharger ->
		{
